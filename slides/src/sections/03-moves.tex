...
\note[item]{Storer p382}
\note[item]{works on right hand too}
...
\note[item]{the first 2 moves makes the star}
\note[item]{pick moves can be really hard to perform in reality, e.g. apply 3rd one after 1 and 2}
...
\note[item]{when $F$ moves towards, the new twist is a twist towards}
\note[item]{when $F$ moves away, the new twist is away, e.g. $L5:R5$ and pick with $R1$}
...
\note[item]{note that we can check if a move is valid since we know how to identify all the segments in a seq}
...
\note[item]{(2) under because our finger is going above}
