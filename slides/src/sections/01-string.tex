...
\note<3>[item]{the native inhabitants in the arctic region play string figures for entertainment }
\note<4>[item]{the indigenous people in New Zealand play string figures for storytelling and illustrating scenes from myths and legends}
\note<4>[item]{and i play string figures when overleaf takes forever to compile my slides}
\end{frame}
...
\note<4>[item]{(take steps) for example, to make the star, we start with this initial position, and then apply two moves of picking a segment of the string}
\note<9>[item]{(mot) clear way of describing how to make string figures}
\note<10>[item]{(mot) teach computers how to play string figures}
\note<10>[item]{(record) store in computer, as database}
\note<10>[item]{simliar to music scores, rubiks cubes}
\note<10>[item]{one effective way of describing string figures is to draw them, as string diagrams}
